 ...
 * Implementa RF006 (múltiples análisis predictivos) y RF007 (visualizaciones
 * gráficas)
 * 
...
public class SmartPrediccionController {
     ...
     * Implementa RF006: Análisis predictivos múltiples con selección automática de
     * algoritmos
     */
    @PostMapping("/inteligente")
    // DESARROLLO: Autorización deshabilitada temporalmente para pruebas con curl
    // TODO: Habilitar en producción - @PreAuthorize("hasAnyRole('GERENTE',
    // 'OPERARIO')")
    @Operation(summary = "Predicción inteligente con Smile ML", description = "Ejecuta predicción avanzada con selección automática de algoritmos. "
            +
    ...
    @Operation(summary = "Calcular horizonte automático", description = "Calcula el horizonte de predicción óptimo basado en: lead time del proveedor, "
            +
    ...
    @PreAuthorize("hasAnyRole('GERENTE', 'OPERARIO')")
    @Operation(summary = "Información de algoritmos Smile ML", description = "Retorna información detallada sobre los algoritmos de Machine Learning disponibles.")
    ...
    @PreAuthorize("hasAnyRole('GERENTE', 'OPERARIO')")
    @Operation(summary = "Datos para visualización gráfica", description = "Obtiene datos históricos formateados para visualizaciones gráficas sin ejecutar predicción. "
            +
    ...
    public ResponseEntity<?> obtenerDatosVisualizacion(
            @Parameter(description = "ID del producto", required = true) @PathVariable Long productoId,
            @Parameter(description = "Días de historia a incluir", example = "90") @RequestParam(defaultValue = "90") Integer diasHistoria) {
    ...
    @PreAuthorize("hasAnyRole('GERENTE', 'OPERARIO')")
    @Operation(summary = "Procesar productos con alertas", description = "Genera predicciones inteligentes para todos los productos con alertas de inventario")
    ...
    @PreAuthorize("hasAnyRole('GERENTE', 'OPERARIO')")
    @Operation(summary = "Validar datos históricos", description = "Verifica si un producto tiene suficientes datos para predicción ML")
    public ResponseEntity<?> validarDatosProducto(
        ...
        try {
            ...
            Map<String, Object> respuesta = Map.of(
                    ...
                    "recomendacion",
                    esValido ? "Producto válido para predicción inteligente" : "Se requieren más datos históricos",
                    "timestamp", java.time.LocalDateTime.now());
    ...
    private Map<String, Object> obtenerDatosHistoricosParaGraficos(Long productoId, Integer diasHistoria) {
        try {
            ...
            datos.put("recomendacionVisualizacion",
                    "Use gráfico de líneas para tendencias, barras para comparaciones estacionales");
    ...
    @Operation(summary = "Predicción automática (PÚBLICO - Solo para pruebas)", description = "Ejecuta predicción inteligente automática para un producto específico. "
            +
    ...
    public ResponseEntity<?> prediccionAutomatica(
            @Parameter(description = "ID del producto a predecir", required = true, example = "1") @PathVariable Long productoId) {
        ...
        try {
            ...
            log.debug(
                    "[PRUEBA_PÚBLICA] Configuración automática: horizonte={} días (calculado con autocorrelación SMILE)",
                    horizonteAutomatico);
            ...
            log.info(
                    "[PRUEBA_PÚBLICA] Predicción completada para producto {}: Algoritmo={}, Horizonte={} días, Calidad={}",
            ...
            log.warn("[PRUEBA_PÚBLICA] Advertencia: Datos insuficientes para producto {}: {}", productoId,
                    e.getMessage());
            return ResponseEntity.badRequest().body(Map.of(
            ...
            log.error("[PRUEBA_PÚBLICA] Error en predicción automática para producto {}: {}", productoId,
                    e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
     ...
     */
    @GetMapping("/estado")
    @Operation(summary = "Estado del servicio", description = "Verifica el estado y disponibilidad del módulo de predicción inteligente")
