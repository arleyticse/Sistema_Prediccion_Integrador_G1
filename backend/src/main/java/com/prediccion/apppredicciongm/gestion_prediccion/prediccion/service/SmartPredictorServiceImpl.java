 ...
 * 
 * Esta versión está completamente integrada con los nuevos DTOs y la interfaz ISmartPredictorService.
 * Implementa RF006 (análisis predictivos múltiples) y RF007 (visualizaciones gráficas).
 */
@Slf4j
...
public class SmartPredictorServiceImpl implements ISmartPredictorService {
    ...
    public SmartPrediccionResponse generarPrediccionInteligente(SmartPrediccionRequest request) {
        ...
        try {
            ...
            int minimoRegistros = request.getMinimoRegistrosHistoricos() != null ? 
                request.getMinimoRegistrosHistoricos() : 10;
            ...
            if (!validarDatosHistoricosProducto(request.getIdProducto(), minimoRegistros)) {
                ...
                throw new IllegalStateException(
                    String.format("Datos históricos insuficientes para producto %d: tiene %d registros, se requieren mínimo %d",
    ...
    /**
     * Calcula el horizonte óptimo de predicción usando autocorrelación con SMILE ML.
     * 
    ...
    /**
     * Selecciona automáticamente el mejor algoritmo basándose en características de los datos
     * 
     * Criterios de selección para minimarket:
     * 1. Datos insuficientes (< 10): LINEAR_REGRESSION (más estable)
     * 2. Datos limitados (< 20): LINEAR_REGRESSION (evita overfitting)
     * 3. Alta variabilidad (CV > 0.7): GRADIENT_BOOSTING (productos perecederos)
     * 4. Estacionalidad detectada: RANDOM_FOREST (patrones complejos)
     * 5. Variabilidad moderada (0.3 ≤ CV ≤ 0.7): RANDOM_FOREST
     * 6. Baja variabilidad (CV < 0.3): LINEAR_REGRESSION (demanda estable)
     */
    private String seleccionarMejorAlgoritmo(double[] serie) {
        ...
        // REGLA 4: Variabilidad baja-moderada → ARIMA (mejor para series temporales con autocorrelación)
        if (variabilidad >= 0.20 && variabilidad <= 0.5 && serie.length >= AlgoritmoSmileML.ARIMA.getMinimoRegistrosRequeridos()) {
            log.info("Seleccionado ARIMA: variabilidad baja-moderada (CV={}), serie temporal", String.format("%.3f", variabilidad));
        ...
        if (variabilidad > 0.5 && variabilidad <= 0.7) {
            if (serie.length >= AlgoritmoSmileML.RANDOM_FOREST.getMinimoRegistrosRequeridos()) {
                log.info("Seleccionado RANDOM_FOREST: variabilidad moderada-alta (CV={})", String.format("%.3f", variabilidad));
                return AlgoritmoSmileML.RANDOM_FOREST.getCodigo();
            } else {
        ...
        // DEFAULT: ARIMA (mejor para series temporales)
        if (serie.length >= AlgoritmoSmileML.ARIMA.getMinimoRegistrosRequeridos()) {
            log.info("Seleccionado ARIMA: algoritmo predeterminado para series temporales");
            return AlgoritmoSmileML.ARIMA.getCodigo();
        }
        
        // FALLBACK: LINEAR_REGRESSION
        log.info("Seleccionado LINEAR_REGRESSION: datos insuficientes para ARIMA");
        return AlgoritmoSmileML.LINEAR_REGRESSION.getCodigo();
    }
    ...
    private ResultadoMLInterno ejecutarPrediccionML(double[] serie, String algoritmo, SmartPrediccionRequest request) {
        ...
        try {
            ...
            log.info("Primeros 10 valores serie: {}", Arrays.stream(Arrays.copyOfRange(serie, 0, Math.min(10, serie.length))).mapToObj(String::valueOf).collect(java.util.stream.Collectors.joining(", ")));
            log.info("Últimos 10 valores serie: {}", Arrays.stream(Arrays.copyOfRange(serie, Math.max(0, serie.length - 10), serie.length)).mapToObj(String::valueOf).collect(java.util.stream.Collectors.joining(", ")));
            ...
            double[] validationSerie = validationSize > 0 ? Arrays.copyOfRange(serie, trainSize, serie.length) : new double[0];
            
            if (validationSize > 0) {
                log.info("Validation set - Primeros 5 valores reales: {}", Arrays.stream(Arrays.copyOfRange(validationSerie, 0, Math.min(5, validationSize))).mapToObj(String::valueOf).collect(java.util.stream.Collectors.joining(", ")));
            }
            
            // Preparar datos temporales para entrenamiento (solo tiempo, sin features que causen data leakage)
            ...
            switch (algoritmoEnum) {
                case LINEAR_REGRESSION:
                    ...
                    if (validationSize > 0) {
                        prediccionesValidacion = ejecutarPrediccionValidacion(dataFrame, trainSerie, validationSize, algoritmoEnum);
                    }
                    break;
                    
                case ARIMA:
                    // ARIMA implementado como OLS con lag features (características de retraso temporal)
     ...
     */
    private double[] ejecutarPrediccionValidacion(DataFrame trainDataFrame, double[] trainSerie, int validationSize, AlgoritmoSmileML algoritmo) {
        try {
            ...
            switch (algoritmo) {
                ...
                case GRADIENT_BOOSTING:
                    GradientTreeBoost modeloGB = GradientTreeBoost.fit(formula, trainDataFrame,
                        new Properties() {{
                            setProperty("smile.gbt.trees", String.valueOf(Math.min(200, Math.max(100, trainSerie.length))));
    ...
    private List<Double> ejecutarRandomForest(DataFrame dataFrame, double[] serie, int horizonte) {
        log.debug("[PREDICCION] Ejecutando Random Forest Regression");
        
        Formula formula = Formula.lhs("demanda");
        
        // Configuración optimizada para minimarket
        int numArboles = Math.min(100, Math.max(50, serie.length / 2)); // 50-100 árboles
        int maxDepth = 10; // Profundidad máxima del árbol
        int maxNodes = 100; // Número máximo de nodos
        int nodeSize = 5; // Tamaño mínimo de nodo
        int mtry = 1; // Solo tiempo como feature
        double subsample = 1.0; // Usar todos los datos
    ...
    /**
     * Implementación de Gradient Boosting Machine
    ...
    private List<Double> ejecutarGradientBoosting(DataFrame dataFrame, double[] serie, int horizonte) {
        log.debug("[PREDICCION] Ejecutando Gradient Boosting Regression");
        
        Formula formula = Formula.lhs("demanda");
        
        // Configuración optimizada para productos perecederos usando Properties
        java.util.Properties props = new java.util.Properties();
        props.setProperty("smile.gbt.trees", String.valueOf(Math.min(200, Math.max(100, serie.length))));
        props.setProperty("smile.gbt.max.depth", "6");
        props.setProperty("smile.gbt.max.nodes", "50");
        props.setProperty("smile.gbt.node.size", "5");
        props.setProperty("smile.gbt.shrinkage", "0.05");
        props.setProperty("smile.gbt.sample.rate", "0.7");
    ...
    private SmartPrediccionResponse construirRespuesta(Producto producto, SmartPrediccionRequest request, ResultadoMLInterno resultado) {
        
        // Generar predicciones detalladas
        ...
        for (int i = 0; i < resultado.getPredicciones().size(); i++) {
            ...
            Double demanda = resultado.getPredicciones().get(i);
            
            prediccionesDetalladas.add(SmartPrediccionResponse.PrediccionDetalle.builder()
                .periodo(i + 1)
                .fecha(fecha.toString())
                .demandaPredicha(demanda)
                .intervaloCorrfianzaInferior(demanda * 0.9)
                .intervaloConfianzaSuperior(demanda * 1.1)
                .build());
        }
        
        // Calcular demanda total
        double demandaTotal = resultado.getPredicciones().stream().mapToDouble(Double::doubleValue).sum();
        ...
        SmartPrediccionResponse response = SmartPrediccionResponse.builder()
            ...
            .estacionalidad(SmartPrediccionResponse.EstacionalidadInfo.builder()
                .tieneEstacionalidad(request.getDetectarEstacionalidad())
            ...
            .riesgoQuiebre(SmartPrediccionResponse.RiesgoQuiebreInfo.builder()
                ...
                .build())
            .ordenCompraSugerida(request.getGenerarOrdenCompra() ? 
                SmartPrediccionResponse.OrdenCompraSugerida.builder()
                    .sugerirOrden(true)
    ...
    private EstacionalidadMensual analizarEstacionalidadMensual(Long idProducto) {
        try {
            ...
            log.warn("[ESTACIONALIDAD] Advertencia: Error analizando estacionalidad mensual para producto {}: {}", idProducto, e.getMessage());
            return EstacionalidadMensual.builder()
                .mesMayor(null)
                .mesMenor(null)
                .intensidad(0.0)
    ...
    private String construirRazonSeleccionAutomatica(String algoritmo, Long idProducto) {
        try {
            List<RegistroDemanda> datosHistoricos = obtenerDatosHistoricos(idProducto);
            double[] serie = prepararSerieTemporal(datosHistoricos);
            ...
            switch (algoritmo) {
                case "RANDOM_FOREST":
                    if (tieneEstacionalidad) {
                        razon.append("Datos con patrones estacionales detectados (CV=")
                             .append(String.format("%.2f", variabilidad))
                ...
                case "LINEAR_REGRESSION":
                    if (serie.length < 10) {
                        ...
                        razon.append("Tendencia lineal clara detectada (coef=")
            ...
            return String.format("Algoritmo %s seleccionado automáticamente basado en características de los datos", algoritmo);
        }
    }
    ...
    private boolean detectarEstacionalidad(double[] serie) {
        if (serie.length < 14) return false;
        
        // Análisis simple de autocorrelación para patrones semanales
        int lag = 7;
        if (serie.length <= lag) return false;
    ...
    private double calcularMAPE(double[] actual, double[] prediccion) {
        ...
        if (countValidos == 0) return 100.0;
    ...
    private double calcularConfianza(double mape) {
        ...
        if (mape < 10) return 0.95 - (mape / 100.0);
        if (mape < 20) return 0.85 - ((mape - 10) / 100.0);
        if (mape < 30) return 0.75 - ((mape - 20) / 100.0);
        if (mape < 50) return 0.65 - ((mape - 30) / 200.0);
        return 0.50;
    }
    ...
    private String clasificarCalidad(double mape) {
        if (mape < 10) return "EXCELENTE";  // Error menor a 10%
        if (mape < 20) return "BUENA";      // Error entre 10-20%
        if (mape < 30) return "REGULAR";    // Error entre 20-30%
    ...
    private List<Double> ejecutarTimeSeriesRegression(double[] serie, int horizonte) {
        log.debug("[PREDICCION] Ejecutando Time Series Regression con lag features");
        
        try {
            // Configuración de lags: usamos últimos 7 valores (autocorrelación semanal)
            int numLags = Math.min(7, serie.length / 4); // Máximo 7 lags, mínimo 25% de datos
            
            // Crear matriz de features con lags: cada fila = [lag1, lag2, ..., lagN, demanda]
            ...
            log.debug("[PREDICCION] Time Series Regression completado: {} predicciones con {} lags", predicciones.size(), numLags);
            return predicciones;
     ...
     * Guarda algoritmo, horizonte, métricas y usuario asociado cuando está disponible.
     * 
    ...
    private Prediccion persistirPrediccion(Producto producto,
        ...
        if (prediccionExistente.isPresent()) {
            ...
            log.info("[PREDICCION] Actualizando predicción existente ID {} para producto {} (Algoritmo: {}, Horizonte: {})", 
