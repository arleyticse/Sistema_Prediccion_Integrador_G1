...
public class ProductoImportacionService implements IProductoImportacionService {
    ...
    public List<String> validarFormatoCSV(MultipartFile archivo) throws IOException {
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(
            ...
            if (headers.length != COLUMNAS_ESPERADAS) {
                errores.add(String.format("Se esperaban %d columnas, se encontraron %d", 
                        COLUMNAS_ESPERADAS, headers.length));
            }
    ...
    private List<ProductoImportacionRequest> leerCSV(MultipartFile archivo) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            for (int i = 1; i < lineas.size(); i++) {
                ...
                if (valores.length == 1 && valores[0].trim().isEmpty()) {
                    continue;
                }

                try {
                    ProductoImportacionRequest request = parsearLineaCSVArray(valores, i + 1);
                    ...
                    errorRequest.setNumeroFila(i + 1);
    ...
    private ProductoImportacionRequest parsearLineaCSVArray(String[] valores, int numeroFila) {
        if (valores.length != COLUMNAS_ESPERADAS) {
            throw new IllegalArgumentException(
                    String.format("Se esperaban %d columnas, se encontraron %d", 
                            COLUMNAS_ESPERADAS, valores.length));
        }
        ...
        try {
            return ProductoImportacionRequest.builder()
                    .numeroFila(numeroFila)
                    .nombre(valores[0].trim())
                    .costoAdquisicion(parseBigDecimal(valores[1]))
                    .costoMantenimiento(parseBigDecimal(valores[2]))
                    .costoMantenimientoAnual(valores[3].trim().isEmpty() ? 
                            BigDecimal.ZERO : parseBigDecimal(valores[3]))
                    .costoPedido(parseBigDecimal(valores[4]))
                    .diasLeadTime(Integer.parseInt(valores[5].trim()))
                    .nombreCategoria(valores[6].trim())
                    .abreviaturaUnidadMedida(valores[7].trim())
                    .build();
